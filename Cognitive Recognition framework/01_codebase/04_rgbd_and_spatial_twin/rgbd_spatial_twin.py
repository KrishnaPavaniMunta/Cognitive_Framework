...
def insert_spatial_memory(conn: sqlite3.Connection, timestamp: str, class_name: str, tracker_id: int, xyz: tuple[float, float, float]) -> None:
    conn.execute(
        f"INSERT INTO {TABLE_NAME} (timestamp, class_name, tracker_id, X, Y, Z) VALUES (?, ?, ?, ?, ?, ?)",
        (timestamp, class_name, tracker_id, xyz[0], xyz[1], xyz[2]),
    )
    conn.commit()
    ...
    try:
        for i, frame in enumerate(frames):
            ...
            if output_video is not None and writer is None:
                ...
                fps = 30.0
                writer = cv2.VideoWriter(output_video, fourcc, fps, video_frame_size)
...
def main() -> None:
    ...
    parser.add_argument("--db", type=str, default=DB_DEFAULT, help="SQLite database path")
