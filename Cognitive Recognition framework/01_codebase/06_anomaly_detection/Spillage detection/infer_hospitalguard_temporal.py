...
# ── Grounded-SAM (optional — graceful fallback if not installed) ──────────────
try:
    import sys as _sys
    _sys.path.insert(0, str(Path(__file__).parent / "sam_masker"))
    from grounded_sam import GroundedSAMRefiner
    _SAM_MODULE_AVAILABLE = True
except Exception as _sam_import_err:
    GroundedSAMRefiner = None  # type: ignore
    _SAM_MODULE_AVAILABLE = False
    print(f"  [SAM] optional module unavailable ({_sam_import_err}) — door masking disabled")
...
EXCEL_PATH = REPO_ROOT / "04_outputs_runs_and_logs/AD_Rules_Outputs/hospitalguard_log.xlsx"
...
# Ego-motion compensation for held egress boxes (robot/camera movement).
EGOMOTION_COMPENSATE_HELD_BOXES = True
EGOMOTION_MAX_SHIFT_PX = 120.0
...
# ── Grounded-SAM config ───────────────────────────────────────────────────────
# Set SAM_ENABLED = True and provide the path to a downloaded SAM checkpoint to
# enable pixel-tight door mask refinement on every DINO frame.
...
def _estimate_background_shift(prev_gray: np.ndarray, curr_gray: np.ndarray) -> tuple[float, float]:
    """
    Estimate dominant frame-to-frame background translation using sparse optical flow.
    Returns (dx, dy): how much previous-frame content moved in current frame.
    """
    ...
    deltas = (next_pts[valid] - prev_pts[valid]).reshape(-1, 2)
    dx = float(np.median(deltas[:, 0]))
    dy = float(np.median(deltas[:, 1]))

    # Reject implausibly large jumps from noisy flow.
    if abs(dx) > EGOMOTION_MAX_SHIFT_PX:
        dx = 0.0
    if abs(dy) > EGOMOTION_MAX_SHIFT_PX:
        dy = 0.0
    return dx, dy


def _shift_persistent_egress_boxes(
    egress_state: dict,
    dx: float,
    dy: float,
    frame_w: int,
    ...
    """Translate held egress boxes using estimated camera/background motion."""
    if abs(dx) < 0.25 and abs(dy) < 0.25:
        return

    for cls in ("door", "exit_sign"):
        shifted = []
        for entry in egress_state.get(cls, []):
...
def run_video(v1: YOLO, v3: YOLO, video_path: Path, out_path: Path) -> tuple[dict, str]:
    ...
    # ── Grounded-SAM: lazy-init on first DINO fire if enabled ────────────────
    _sam_refiner: "GroundedSAMRefiner | None" = None
    if SAM_ENABLED and _SAM_MODULE_AVAILABLE and SAM_CKPT_PATH.exists():
        try:
            _sam_refiner = GroundedSAMRefiner(
                ckpt_path=SAM_CKPT_PATH,
                model_type=SAM_MODEL_TYPE,
                device=SAM_DEVICE,
            )
        except Exception as _e:
            print(f"  [SAM] init failed: {_e} — continuing without SAM")
            _sam_refiner = None
    elif SAM_ENABLED:
        print("  [SAM] enabled but checkpoint not found at "
              f"{SAM_CKPT_PATH} — door masking skipped")
    ...
    while True:
        ...
        if EGOMOTION_COMPENSATE_HELD_BOXES:
            curr_gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
            if prev_gray is not None:
                shift_dx, shift_dy = _estimate_background_shift(prev_gray, curr_gray)
                _shift_persistent_egress_boxes(egress_state, shift_dx, shift_dy, width, height)
            prev_gray = curr_gray
        ...
        if is_glare and last_tracked_sv is not None:
            ...
            if (frame_idx - 1) % dino_frame_interval == 0:
                ...
                # ── Grounded-SAM: refine DINO door boxes to pixel-tight masks ──
                # Runs at the same cadence as DINO (every N frames).
                # Replaces loose DINO boxes with SAM-snapped tight boxes;
                # the polygon contour is stored at index[5] for overlay drawing.
                if _sam_refiner is not None and "door" in active_dino:
                    sam_refined = _sam_refiner.refine_boxes(
                        bgr, active_dino["door"], frame_id=frame_idx
                    )
                    if sam_refined:
                        # Drop polygon element — keep only (x1,y1,x2,y2,conf)
                        # so downstream code (NMS, tracker) sees a plain 5-tuple.
                        active_dino["door"] = [
                            (r[0], r[1], r[2], r[3], r[4])
                            for r in sam_refined
                        ]
            ...
            # Aggressively collapse fragmented door detections before tracking.
            if "door" in all_dets and len(all_dets["door"]) > 1:
                all_dets["door"] = _nms_merge(all_dets["door"], iou_thresh=0.30)
                all_dets["door"].sort(key=lambda d: d[4], reverse=True)
                all_dets["door"] = all_dets["door"][:2]
...
def run_image(v1: YOLO, v3: YOLO, media_path: Path,
    ...
    # Aggressively collapse fragmented door detections before annotation.
    if "door" in all_dets and len(all_dets["door"]) > 1:
        all_dets["door"] = _nms_merge(all_dets["door"], iou_thresh=0.30)
        all_dets["door"].sort(key=lambda d: d[4], reverse=True)
        all_dets["door"] = all_dets["door"][:2]
