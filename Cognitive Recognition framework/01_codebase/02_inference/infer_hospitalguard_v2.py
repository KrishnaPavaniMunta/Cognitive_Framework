...
# Stricter anti-FP options for video mode.
# 1) Require anchors for every class that defines anchors.
# 2) Temporal confirmation for DINO detections across refreshes.
DINO_REQUIRE_ANCHOR_FOR_ALL = True
DINO_TEMPORAL_IOU = 0.30
DINO_TEMPORAL_MIN_HITS = 2
DINO_TEMPORAL_MIN_HITS_STRICT = 3
DINO_TEMPORAL_MAX_GAP = 2
DINO_TEMPORAL_INSTANT_CONF = 0.78
DINO_TEMPORAL_INSTANT_CONF_STRICT = 0.86
DINO_TEMPORAL_STRICT_CLASSES: set[str] = {
    "glove", "mask", "hair_net", "surgical_light",
    "iv_stand", "exit_sign", "test_tube", "medical_tray", "infusion_pump",
}
...
def _context_gate(missing_classes: list[str], yolo_dets: dict) -> list[str]:
    ...
    for cls in missing_classes:
        ...
        if anchors and (cls in DINO_GATED_CLASSES or DINO_REQUIRE_ANCHOR_FOR_ALL):
...
def _temporal_filter_dino(raw_dino: dict, state: dict, refresh_idx: int) -> dict:
    """
    Suppress one-off DINO hallucinations by requiring temporal consistency.
    A detection is emitted if either:
      - confidence is very high (instant pass), or
      - it matches across refreshes for enough consecutive hits.
    """
    # Prune stale tracks globally.
    for cls in list(state.keys()):
        tracks = [
            t for t in state[cls]
            if (refresh_idx - t["last_seen"]) <= DINO_TEMPORAL_MAX_GAP
        ]
        if tracks:
            state[cls] = tracks
        else:
    ...
    for cls, dets in raw_dino.items():
        tracks = state.setdefault(cls, [])
        ...
        for det in dets_sorted:
            ...
            for i, tr in enumerate(tracks):
                if i in used_track_ids:
                    continue
                gap = refresh_idx - tr["last_seen"]
                if gap > DINO_TEMPORAL_MAX_GAP:
                    continue
                iou = _box_iou(det, tr["box"])
                if iou > best_iou:
                    best_i, best_iou = i, iou

            if best_i >= 0 and best_iou >= DINO_TEMPORAL_IOU:
                tr = tracks[best_i]
                tr["box"] = det
                tr["last_seen"] = refresh_idx
                tr["hits"] += 1
                tr["best_conf"] = max(tr["best_conf"], float(det[4]))
                used_track_ids.add(best_i)
            else:
                tr = {
                    "box": det,
                    "last_seen": refresh_idx,
                    "hits": 1,
                    "best_conf": float(det[4]),
                }
            updated_tracks.append(tr)

        # Keep tracks we updated plus recently seen tracks.
        survivors = updated_tracks + [
            tr for i, tr in enumerate(tracks)
            if i not in used_track_ids and (refresh_idx - tr["last_seen"]) <= DINO_TEMPORAL_MAX_GAP
        ]
        state[cls] = survivors

        min_hits = DINO_TEMPORAL_MIN_HITS_STRICT if cls in DINO_TEMPORAL_STRICT_CLASSES else DINO_TEMPORAL_MIN_HITS
        instant_conf = DINO_TEMPORAL_INSTANT_CONF_STRICT if cls in DINO_TEMPORAL_STRICT_CLASSES else DINO_TEMPORAL_INSTANT_CONF

        keep = []
        for tr in survivors:
            conf = float(tr["box"][4])
            if conf >= instant_conf or tr["hits"] >= min_hits:
                keep.append(tr["box"])
        if keep:
            filtered[cls] = keep
    ...
    while True:
        ...
        if (frame_idx - 1) % dino_frame_interval == 0:
            ...
            if missing_weak:
                ...
                raw_dino = dino_infer(pil_img, missing_weak)
            active_dino = _temporal_filter_dino(raw_dino, dino_temporal_state, dino_refresh_idx)
            dino_refresh_idx += 1
