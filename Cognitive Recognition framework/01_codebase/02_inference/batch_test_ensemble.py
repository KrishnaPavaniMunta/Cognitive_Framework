"""
batch_test_ensemble.py
Batch tests 20 bag + 20 spillage + 40 exit_sign images using the V1+V3 ensemble.
Appends results to outputs/v3_inference_log.xlsx (same sheet as infer_ensemble.py).
Saves annotated images to outputs/inference_output/.
"""
...
# ── Batch config: (class_label, n_images, [openverse_queries]) ────────────────
BATCH = [
    ("bag",       20, ["shopping bag", "carrier bag", "plastic bag tote"]),
    ("spillage",  20, ["wet floor", "puddle water", "spill water", "flood floor"]),
    ("exit_sign", 40, ["exit sign", "emergency exit sign", "fire exit sign"]),
]
...
def draw_merged(img_path, merged_dets):
    ...
    for cls, dets in merged_dets.items():
        ...
        for (x1, y1, x2, y2, conf) in dets:
            ...
            (tw, th), bl = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, scale, thick)
            ty = max(y1 - 4, th + 4)
            cv2.rectangle(img, (x1, ty - th - 4), (x1 + tw + 4, ty + bl), (0, 0, 0), -1)
...
def fetch_image_urls(queries, n):
    """Return up to n image URLs via Openverse, iterating multiple short queries."""
    if isinstance(queries, str):
        queries = [queries]
    urls, seen = [], set()
    want = n + 30
    session = requests.Session()
    ua = {"User-Agent": "YoloEnsembleTest/1.0 (research)"}
...
total_done = 0
summary = {}   # class -> {tp, tp_low, fn, fp, tn}

for class_label, n_target, queries in BATCH:
    print(f"{'='*60}")
    print(f"Class: {class_label}  |  target: {n_target} images")
    print(f"Queries: {queries}")

    urls = fetch_image_urls(queries, n_target)
    if not urls:
        print(f"  [!] No URLs found for {class_label}, skipping.")
        continue

    stats = {"TP": 0, "TP (Low Conf)": 0, "FN (False Negative)": 0,
             "FP (False Positive)": 0, "TN (True Negative)": 0}
    done = 0
    url_idx = 0

    while done < n_target and url_idx < len(urls):
        url = urls[url_idx]
        url_idx += 1
        ...
        try:
            ...
            print(f"  [{done+1}/{n_target}] SKIP (download): {e}")
            continue

        try:
            merged = ensemble_infer(v1, v3, img_path)
        except Exception as e:
            print(f"  [{done+1}/{n_target}] SKIP (infer): {e}")
            os.unlink(img_path)
            continue

        flat_dets = {cls: [d[4] for d in dets] for cls, dets in merged.items()}
        detected_str, conf_str, result_type, notes = classify_result(class_label, flat_dets)

        # Prepend source tag
        if detected_str == "Yes":
            tgt_key = next((k for k in flat_dets if k.lower() == class_label.lower()), None)
            if tgt_key:
                src = f"[{source_label(tgt_key)}] "
                notes = src + notes if notes else src.rstrip()

        ts       = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_name = OUT_DIR / f"{class_label}_{done+1:02d}_{ts}.jpg"
        ann      = draw_merged(img_path, merged)
        if ann is not None:
            cv2.imwrite(str(out_name), ann)

        write_row(ws, col_map, excel_row, url, class_label, detected_str,
                  conf_str, result_type, notes, flat_dets)
        wb.save(EXCEL_PATH)
        ...
        det_short = f"{class_label}:{conf_str}" if detected_str == "Yes" else "—"
        print(f"  [{done:02d}/{n_target}] {result_type:<22}  {det_short}")

        # Small delay to stay within Wikimedia rate limits
        time.sleep(0.8)

    summary[class_label] = {"done": done, "stats": stats}
    if done < n_target:
        print(f"  [!] Only completed {done}/{n_target} (ran out of URLs).")

# ── Summary ───────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print(f"BATCH COMPLETE  —  {total_done} images processed")
print(f"Excel: {EXCEL_PATH}")
print()
for cls, info in summary.items():
    s = info["stats"]
    tp  = s.get("TP", 0) + s.get("TP (Low Conf)", 0)
    fn  = s.get("FN (False Negative)", 0)
    total = info["done"]
    pct = 100 * tp / total if total else 0
    print(f"  {cls:<12}  {total:>2} images  |  TP={tp}  FN={fn}  "
          f"TP(Low)={s.get('TP (Low Conf)',0)}  "
          f"FP={s.get('FP (False Positive)',0)}  "
          f"detect rate={pct:.0f}%")
